...
const PainPoints: React.FC = () => {
  ...
  return (
    <section id="solutions" className="py-32 relative overflow-hidden animate-section-fade-in">
      ...
      <div className="absolute top-0 right-0 w-1/3 h-full bg-gradient-to-l from-orange-500/10 to-transparent -z-10"></div>
        ...
        <div className="grid md:grid-cols-3 gap-8">
          {points.map((p, i) => (
            <div key={i} className="group relative">
              <div className="absolute inset-0 bg-gray-900 rounded-[2.5rem] shadow-xl translate-y-2 group-hover:translate-y-4 transition-transform duration-300 -z-10"></div>
              <div className="relative glass p-10 rounded-[2.5rem] border-white/5 h-full flex flex-col justify-between transition-all duration-300 hover:-translate-y-1 hover:shadow-2xl hover:bg-gray-900/40">
                <div>
                  <div className="flex justify-between items-start mb-8">
                    <div className="w-16 h-16 bg-gray-800 border border-gray-700 rounded-2xl flex items-center justify-center group-hover:scale-110 group-hover:border-orange-500/50 transition-all duration-300 shadow-sm">
                      <i className={`fas ${p.icon} text-2xl text-gray-400 group-hover:text-orange-500 transition-colors`}></i>
                    </div>
                ...
                <div className="mt-8 pt-8 border-t border-white/5">
                  <span className="text-orange-500 font-bold text-sm flex items-center gap-2 group-hover:translate-x-2 transition-transform cursor-pointer">
